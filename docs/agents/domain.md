...
- **`CONTEXT.md`** at the repo root (`src/brave/CONTEXT.md`).
...
customization layer). The domain scope for `CONTEXT.md` here is this fork's own
work — the persona, profile proxy, fingerprint spoofing, and L3 geo-consistency
work on the `fingerprint` branch — not an attempt to model all of brave-core or
Chromium.
