...
- [x] 5.2 实现 Profile 级 `FingerprintProxyService` 并接入
      `proxy_config_monitor.cc`，承载 HTTP/HTTPS/SOCKS5 server + 凭证；Tor-first
- [x] 5.3 验证/接线 SOCKS5 认证：复用
      `SOCKS5ClientSocketAuth`（`socks5_client_socket.cc`）
...
- [x] 5.6 browsertest：Profile
      ...
      Profile 行为不受注入缝扩展影响**
- [x] 5.7 检测运行时代理认证失败（HTTP 407 反复失败 / SOCKS5
      RFC1929 拒绝）并经设置页给出可见错误反馈（对应 spec「认证失败可见反馈」，区别于 7.3 的静态字段校验）；补 browsertest
- [x] 5.8 审计其他
...
- [ ] 10.6 提供真实 HTTP/SOCKS5 `0600` fixture、原崩溃插件
      `FP_QA_PRIMARY_EXTENSION_URL`、批准 baseline/native
      evidence 后执行 Full 与 Soak 并归档 0 失败报告
