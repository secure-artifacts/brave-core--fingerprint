...
async function clickExtensionControl(page, extensionId, selector) {
  ...
  return await page.evaluate(
    ({ extensionId, selector }) => {
      ...
      const item = all(document, 'extensions-item').find(
        (candidate) =>
          candidate.data?.id === extensionId || candidate.id === extensionId,
      )
...
async function waitForEnabled(page, extensionId, expected, timeoutMs = 10000) {
  ...
  throw Object.assign(
    new Error(
      `Extension ${extensionId} enabled state did not become ${expected}`,
    ),
...
async function removeWebStoreExtension(page, extensionId, screenshot, pid) {
  ...
  await page.evaluate((extensionId) => {
    chrome.management
      .uninstall(extensionId, { showConfirmDialog: true })
      .catch(() => {})
  }, extensionId)
...
async function captureExtensionPages(
  page,
  extensionId,
  manifest,
  pageDir,
  ...
  for (const [name, resource] of targets) {
    ...
    const extensionUrl = `chrome-extension://${extensionId}/${resource.replace(/^\//, '')}`
...
function assertWorkerFingerprint(worker, main) {
  ...
  if (
    JSON.stringify(worker.languages) !== JSON.stringify(main.basic.languages)
  ) {
...
export async function runLocalExtensionLifecycle({
  config,
  dirs,
  fixtureDir,
  probe,
  ...
  try {
    ...
    const extension = await waitForExtension(
      page,
      (item) => item.id === loaded.id && item.name === 'Fingerprint QA MV3',
    )
    const mainFingerprint = await collectProbe(page, probe.origin)
    const workerFingerprint = await sendExtensionMessage(page, extension.id, {
      type: 'collect',
    })
    ...
    const conflict = await sendExtensionMessage(page, extension.id, {
      type: 'set-proxy-conflict',
    })
    if (conflict?.error) {
    ...
    const cleared = await sendExtensionMessage(page, extension.id, {
      type: 'clear-proxy-conflict',
    })
    if (cleared?.error) {
    ...
    await page.goto('brave://extensions/', { waitUntil: 'domcontentloaded' })
    if (
      !(await clickExtensionControl(page, extension.id, '#dev-reload-button'))
    ) {
      throw new Error('Extension reload control missing')
    }
    await waitForEnabled(page, extension.id, true)
    const reloadedFingerprint = await sendExtensionMessage(page, extension.id, {
      type: 'collect',
    })
    ...
    await session.browserSession.send('Extensions.uninstall', {
      id: extension.id,
    })
    page =
      session.context.pages().find((candidate) => !candidate.isClosed())
    ...
    for (const events of eventSets)
      assertSessionHealth(events, 'Local MV3 lifecycle')
    return {
      extension,
      screenshots,
      workerFingerprint,
...
export async function runWebStoreExtensionLifecycle({
  ...
  try {
    ...
    const confirmationScreenshot = path.join(
      dirs.native,
      `full-cws-${label}-install-confirmation.png`,
    )
    await confirmNativeInstall(
    ...
    try {
      ...
      const blockedScreenshot = path.join(
        dirs.native,
        `full-cws-${label}-install-confirmation-blocked.png`,
      )
      ...
      if (await nativeScreenshotHasText(blockedScreenshot, 'Add extension')) {
        return {
          reason:
            'macOS blocked native QA input; grant Accessibility/Input Monitoring before Chrome Web Store automation',
          screenshots: [confirmationScreenshot, blockedScreenshot],
          status: 'BLOCKED',
        }
      }
      const installError = page.isClosed()
        ? null
        : await page.evaluate(() => globalThis.__fingerprintQaWebStoreError)
      if (installError) {
    ...
    const screenshots = [
      confirmationScreenshot,
      ...(await captureWebStoreExtensionPages({
        page,
        extension: installed,
        manifest,
        dirs,
    ...
    const nativeToolbarScreenshot = path.join(
      dirs.native,
      `full-cws-${label}-installed-toolbar.png`,
    )
    await captureNativeScreenshot(
      nativeToolbarScreenshot,
      session.process.child.pid,
    )
    ...
    const uninstallScreenshot = path.join(
      dirs.native,
      `full-cws-${label}-uninstall-confirmation.png`,
    )
