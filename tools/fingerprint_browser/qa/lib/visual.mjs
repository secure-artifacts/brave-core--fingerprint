...
async function imageStats(image) {
  ...
  const [width, height, red, green, blue, standardDeviation] =
...
async function redPixelRatio(image) {
  const result = await run('magick', [
    ...
    '-alpha', 'on',
    '-colorspace', 'sRGB',
    '-fx', '(a>0.5&&r>0.85&&g<0.18&&b<0.18)?1:0',
    '-format', '%[fx:mean]',
    'info:',
  ], {check: true})
...
async function redComponentStats(image) {
  ...
  const components = `${result.stdout}\n${result.stderr}`.split('\n').flatMap(line => {
    ...
    if (!channels.some(channel => Number.isFinite(channel) && channel > 0.5)) return []
...
export async function analyzeScreenshot({
  ...
  if (path.isAbsolute(baselineKey) || baselineKey.split(path.sep).includes('..')) {
  ...
  if (checkRed) {
    ...
    if (await pathExists(candidateRedMask)) {
      ...
      redSource = await applyMask(
        actual, redMask, path.join(diffDir, `${baselineKey}.red-masked.png`))
    }
  ...
  if (await pathExists(baseline)) {
    ...
    if (baselineStats.width !== stats.width || baselineStats.height !== stats.height) {
      checks.push({
      ...
      if (await pathExists(candidateMask)) {
        ...
        comparedActual = await applyMask(
          actual, mask, `${diff}.actual-masked.png`)
        comparedBaseline = await applyMask(
          baseline, mask, `${diff}.baseline-masked.png`)
      }
      ...
      checks.push(
        {actual: rmse, limit: maxRmse, name: 'baseline-rmse', pass: rmse <= maxRmse},
        {
      ...
      baselineStatus = checks.slice(-3).every(check => check.pass) ? 'PASS' : 'FAIL'
    }
  }
  return {
    actual,
    ...
    reason: checks.every(check => check.pass)
      ...
      : checks.filter(check => !check.pass)
        .map(check => check.expected
          ...
          : `${check.name} ${check.actual} outside limit ${check.limit}`).join('; '),
...
export async function loadHumanVisualReview({analyses, artifacts, manifestFile}) {
  if (!manifestFile) {
    return {
  ...
  if (!(await pathExists(manifestFile))) {
    return {reason: `Visual review manifest does not exist: ${manifestFile}`, status: 'BLOCKED'}
  }
  ...
  if (manifest.libchromeSha256 !== artifacts.libchrome.source.sha256 ||
    ...
    return {reason: 'Visual review manifest artifact hashes do not match', status: 'BLOCKED'}
  }
  ...
  const missing = reviews.filter(review =>
    !['PASS', 'FAIL'].includes(review.status) || !String(review.reason || '').trim())
  if (missing.length > 0) {
    return {
  ...
  return {
    ...
    reason: failed.length > 0 ? `${failed.length} screenshots failed human review` : null,
    reviews,
...
function parseColor(value) {
  ...
  return srgb
    ? [...srgb.slice(1, 4).map(channel => Number(channel) * 255), Number(srgb[4] ?? 1)]
    : null
}
...
export function contrastRatio(foreground, background) {
  ...
  const composite = fg.slice(0, 3).map((value, index) =>
    value * fg[3] + bg[index] * (1 - fg[3]))
