...
export const SOURCE_GROUPS = {
  native: [
    ...
    '../third_party/blink/renderer/modules/mediastream/transferred_media_stream_track.cc',
    '../third_party/blink/renderer/modules/mediastream/transferred_media_stream_track.h',
    ...
    '../third_party/blink/renderer/modules/webaudio/media_stream_audio_source_node.cc',
    ...
    'patches/third_party-blink-renderer-core-timezone-timezone_controller.cc.patch',
    'patches/third_party-blink-renderer-core-timezone-timezone_controller.h.patch',
    'patches/third_party-blink-renderer-modules-gamepad-navigator_gamepad.cc.patch',
    'patches/third_party-blink-renderer-modules-mediastream-user_media_request.cc.patch',
    'patches/third_party-blink-renderer-modules-mediastream-media_stream_track.h.patch',
    'patches/third_party-blink-renderer-modules-mediastream-media_stream_track_impl.cc.patch',
    'patches/third_party-blink-renderer-modules-mediastream-media_stream_track_impl.h.patch',
    'patches/third_party-blink-renderer-modules-mediastream-transferred_media_stream_track.cc.patch',
    'patches/third_party-blink-renderer-modules-mediastream-transferred_media_stream_track.h.patch',
    'patches/third_party-blink-renderer-modules-mediastream-webaudio_media_stream_audio_sink.cc.patch',
    'patches/third_party-blink-renderer-modules-mediastream-webaudio_media_stream_audio_sink.h.patch',
...
async function namedFilesUnder(target, name) {
  return (await filesUnder(target)).filter(
...
export async function unsignedMachOSha256(file) {
  ...
  try {
    ...
    const commandRead = await handle.read(
      commands,
      0,
      commandBytes,
      header.length,
    ...
    if (
      signature.dataOffset < header.length + commandBytes
    ...
    let rangeStart = 0
    for (const field of normalizedFields.sort(
      (left, right) => left.offset - right.offset,
    )) {
      if (
        field.offset < rangeStart
        || field.offset + field.size > signature.dataOffset
      ) {
...
export async function writeBuildManifest({ braveRoot, mode, outDir }) {
  ...
  const groups =
    mode === 'cpp'
      ? {
          braveResources: await sourceGroupIdentity(
            braveRoot,
            'braveResources',
          ),
          chromiumResources: await sourceGroupIdentity(
            braveRoot,
            'chromiumResources',
          ),
          localeResources: await sourceGroupIdentity(
            braveRoot,
            'localeResources',
          ),
          native: await sourceGroupIdentity(braveRoot, 'native'),
          network: await sourceGroupIdentity(braveRoot, 'network'),
          scaledResources: await sourceGroupIdentity(
            braveRoot,
            'scaledResources',
          ),
        }
  ...
  if (!groups.native || !groups.network) {
    throw new Error(
      'A cpp build manifest is required before a resources-only update',
    )
  }
...
async function verifyBuildManifest(braveRoot, outDir) {
  ...
  if (JSON.stringify(artifacts) !== JSON.stringify(manifest.artifacts)) {
    throw Object.assign(
      new Error('Build artifact hashes do not match source manifest'),
      {
...
export async function prepareAndVerifyArtifacts(config, log) {
  ...
  if (!(await pathExists(app))) {
    ...
    if (!(await pathExists(developmentApp))) {
      throw new Error(
        `Development app baseline does not exist: ${developmentApp}`,
      )
    }
  ...
  if (sourceAppExecutables.digest !== appExecutables.digest) {
    if (!prepareApp) {
      throw Object.assign(
        new Error(
          'QA app executable set does not match current development app',
        ),
  ...
  const sourceChromiumResource = path.join(
    outDir,
    'gen',
    'repack',
    'resources.pak',
  )
  const appChromiumResource = path.join(
    appVersion,
    'Resources',
    'resources.pak',
  )
  ...
  const initialAppChromiumResources = await namedFilesUnder(
    app,
    'resources.pak',
  )
  ...
  const scaledPackTargets = (
    await Promise.all(
      scaledPackSources.map(async (item) => {
  ...
  if (
    sourceDylibs.length === 0
    || !(await pathExists(sourceResource))
    || !(await pathExists(sourceChromiumResource))
    || sourceLocalePacks.length === 0
    || !(
      await Promise.all(
        scaledPackSources.map((item) => pathExists(item.source)),
      )
    ).every(Boolean)
  ) {
  ...
  const resourceSource = await latestSource(
    braveRoot,
    SOURCE_GROUPS.braveResources,
  )
  const chromiumResourceSource = await latestSource(
    braveRoot,
    SOURCE_GROUPS.chromiumResources,
  )
  const localeSource = await latestSource(
    braveRoot,
    SOURCE_GROUPS.localeResources,
  )
  const scaledResourceSource = await latestSource(
    braveRoot,
    SOURCE_GROUPS.scaledResources,
  )
  ...
  const localeArtifactStats = await Promise.all(
    sourceLocalePacks.map(async (file) => ({
      file,
      stat: await fs.stat(file),
    })),
  )
  const scaledArtifactStats = await Promise.all(
    scaledPackSources.map(async (item) => ({
      file: item.source,
  ...
  const freshness = {
    ...
    chromiumResources: {
      ...
      latestSourceMtime: chromiumResourceSource.mtimeMs
        ...
        : null,
      pass:
        chromiumResourceSource.mtimeMs <= chromiumResourceArtifactStat.mtimeMs,
    },
    locales: {
  ...
  if (
    !freshness.native.pass
    || !freshness.network.pass
    || !freshness.resources.pass
    || !freshness.chromiumResources.pass
    || !freshness.locales.pass
    || !freshness.scaledResources.pass
  ) {
    throw Object.assign(
      new Error('Source artifacts are older than relevant source edits'),
      {
        details: freshness,
      },
    )
  ...
  if (prepareApp) {
    ...
    for (const source of sourceDylibs) {
      ...
      if (
        !current
        || JSON.stringify(sourceUuid) !== JSON.stringify(appUuid)
  ...
  if (missing.length > 0) {
    throw Object.assign(
      new Error(`QA app is missing ${missing.length} dylibs`),
      {
        details: { missing },
      },
    )
  ...
  if (extra.length > 0) {
    throw Object.assign(
      new Error(`QA app has ${extra.length} unexpected dylibs`),
      {
        details: { extra },
      },
    )
  ...
  const comparisons = sourceDylibs.map((source) => {
    ...
    return {
      ...
      match:
        JSON.stringify(sourceUuid) === JSON.stringify(appUuid)
        && JSON.stringify(sourceLoadCommands)
          === JSON.stringify(appLoadCommands)
  ...
  if (mismatched.length > 0) {
    throw Object.assign(
      new Error(`QA app has ${mismatched.length} mismatched dylibs`),
      {
        details: { mismatched },
      },
    )
  ...
  const scaledPackComparisons = await Promise.all(
    scaledPackTargets.map(async (item) => {
  ...
  const localeComparisons = await Promise.all(
    localeTargets.map(async (item) => {
      if (!(await pathExists(item.destination))) {
        return { ...item, match: false, reason: 'missing' }
      }
      const [source, app] = await Promise.all([
  ...
  const mismatchedLocales = localeComparisons.filter(
    (comparison) => !comparison.match,
  )
  if (mismatchedLocales.length > 0) {
    throw Object.assign(
      new Error(
        `${mismatchedLocales.length} QA app locale packs do not match current output`,
      ),
  ...
  const [
    ...
    appResourceRecord,
    appResourceRecords,
    sourceChromiumResourceRecord,
    appChromiumResourceRecord,
    appChromiumResourceRecords,
    sourceLibRecord,
    appLibRecord,
  ] = await Promise.all([
  ...
  const mismatchedResources = appResourceRecords.filter(
    (record) => record.sha256 !== sourceResourceRecord.sha256,
  )
  if (mismatchedResources.length > 0) {
    throw Object.assign(
      new Error(
        `${mismatchedResources.length} QA app resource packs do not match current output`,
      ),
      { details: mismatchedResources },
    )
  }
  const mismatchedChromiumResources = appChromiumResourceRecords.filter(
    (record) => record.sha256 !== sourceChromiumResourceRecord.sha256,
  )
  if (mismatchedChromiumResources.length > 0) {
    throw Object.assign(
      new Error(
        `${mismatchedChromiumResources.length} QA app Chromium resource packs do not match current output`,
      ),
  ...
  const appLibContentSha256 = appContentHashes.get(
    path.resolve(path.join(appDylibDir, 'libchrome_dll.dylib')),
  )
  if (sourceLibContentSha256 !== appLibContentSha256) {
    throw new Error(
      'QA app libchrome_dll.dylib content does not match current output',
    )
  }
