...
async function makeDirectories(runDir) {
  ...
  await Promise.all(Object.values(dirs).map(directory =>
...
async function main() {
  ...
  try {
    const processGate = await runScenario(report, 'preflight-stop-old-qa-processes', async () => {
      const stopped = await stopAllQaProcesses()
      await log(`Stopped ${stopped.length} previous QA profile process groups`)
      const residual = stopped.flatMap(result => result.residual || [])
      if (residual.length > 0) {
        throw Object.assign(new Error(`${residual.length} old QA processes could not be stopped`), {
          details: residual,
        })
      }
      return {stopped}
    ...
    if (config.keepProfile && config.mode !== 'smoke') {
      await runScenario(report, 'preflight-profile-cleanup-required', async () => ({
        reason: 'Full/Soak delivery cannot use --keep-profile',
        status: 'BLOCKED',
      }))
    }

    const artifactScenario = await runScenario(report, 'preflight-current-artifacts', async () => {
    ...
    if (config.mode !== 'smoke') {
      ...
      if (config.skipCodeTests) {
        ...
        codeGates.push(
          await runScenario(report, 'code-tests-unit', async () =>
            await runUnitTests({braveRoot: config.braveRoot, dirs, outDir: config.outDir})),
          await runScenario(report, 'code-tests-net', async () =>
            await runNetTests({braveRoot: config.braveRoot, dirs, outDir: config.outDir})),
          await runScenario(report, 'code-tests-performance-manager', async () =>
            await runPerformanceManagerTests({
              braveRoot: config.braveRoot,
              dirs,
              outDir: config.outDir,
            })),
          await runScenario(report, 'code-tests-browser', async () =>
            await runBrowserTests({braveRoot: config.braveRoot, dirs, outDir: config.outDir})),
        )
      }
    ...
    probe = await startProbeServer(path.join(
      config.braveRoot, 'tools', 'fingerprint_browser', 'qa', 'fixtures', 'probe'))
    ...
    smokeSession = null
    await writeReports(report, runDir)
    if (report.scenarios.slice(smokeScenarioIndex).some(
    ...
    await runScenario(report, 'visual-analysis-all-screenshots', async () => {
      ...
      if (result.failed.length > 0) {
        throw Object.assign(
          new Error(`${result.failed.length} screenshots failed visual analysis`),
      ...
      if (config.mode !== 'smoke') {
        ...
        if (review.status !== 'PASS') {
          return {analyses: result.analyses, review, screenshots, status: review.status}
        }
        return {analyses: result.analyses, review, screenshots}
      }
      return {analyses: result.analyses, screenshots}
    })
  } catch (error) {
    ...
    await runScenario(report, 'postflight-cleanup', async () => {
      ...
      const residual = [
        ...
        ...(await listProcesses()).filter(item =>
          item.command.includes('Brave Browser Development') &&
          item.command.includes('/tmp/fingerprint-browser-')),
      ]
      if (residual.length > 0) {
        throw Object.assign(new Error(`${residual.length} QA processes remained`), {
          details: residual,
        })
      }
    ...
    if (artifactGatePassed) {
      ...
      await runScenario(report, 'postflight-no-fatal-browser-logs', async () => {
        const failures = await scanFatalLogs(dirs.logs)
        if (failures.length > 0) {
          throw Object.assign(new Error(`${failures.length} fatal browser log entries found`), {
            details: failures,
          })
        }
        return {failures}
    ...
    process.exitCode = finalReport.status === 'PASS'
      ? 0
      : finalReport.status === 'BLOCKED' ? 2 : 1
  }
}
