...
test('processesForProfile matches only Development QA command line', () => {
  ...
  const processes = [
    {command: `Brave Browser Development --user-data-dir=${profile}`, pid: 1, ppid: 0},
    {command: 'Brave Browser --user-data-dir=/Users/test/BraveSoftware', pid: 2, ppid: 0},
    {command: 'Brave Browser Development --user-data-dir=/tmp/other', pid: 3, ppid: 0},
    {command: 'Brave Browser Development Helper --type=renderer', pid: 4, ppid: 1},
    {command: 'Brave Browser Development Helper --type=utility', pid: 5, ppid: 4},
    {command: `Brave Browser Development --user-data-dir=${profile}-other`, pid: 6, ppid: 0},
  ]
  assert.deepEqual(processesForProfile(processes, profile).map(item => item.pid), [1, 4, 5])
})
...
test('contrastRatio implements WCAG luminance ratio', () => {
  ...
  assert.equal(contrastRatio(
    'color(srgb 0 0 0 / 1)', 'color(srgb 1 1 1 / 1)'), 21)
...
test('pngDimensions reads IHDR dimensions and rejects invalid files', async () => {
  ...
  try {
    ...
    await fs.writeFile(png, Buffer.from(
