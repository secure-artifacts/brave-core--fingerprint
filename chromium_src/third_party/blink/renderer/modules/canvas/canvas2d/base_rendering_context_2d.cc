...
#define BRAVE_GET_IMAGE_DATA                                                   \
  if (ExecutionContext* context = ExecutionContext::From(script_state)) {      \
    ...
    brave::BraveSessionCache::From(*context).PerturbCanvasPixels(              \
        gfx::SkPixmapToWritableSpan(image_data_pixmap), Width(), Height(), sx, \
        sy, image_data_pixmap.width(), image_data_pixmap.height(),             \
        image_data_pixmap.rowBytes(),                                          \
        static_cast<int>(image_data_pixmap.colorType()));                      \
  }
